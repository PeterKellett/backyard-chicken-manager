...
from .models import EggCollection, Feeds, Disinfectants, FeedingTime, WateringTime
from django.db.models import Count
from .forms import EggCollectionForm, FeedsForm, FeedingTimeForm, WateringTimeForm, CoopCleaningForm
...
def get_flock_feeds(request, flock_id):
    ...
    for x in filtered_feeds:
        print("filtered_feeds = ", x['id'], x['feed_name'], x['date'])
    return JsonResponse(list(filtered_feeds), safe=False)
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            if feed:
                ...
                feed[0].qty_food -= form.feeder_amount  # Update this qty_food value by subtracting it from itself
...
def watering_time(request):
    userprofile = UserProfile.objects.get(user=request.user)
    farmprofile = userprofile.farmprofiles.all()
    if request.POST:
        print("POST")
        form = WateringTimeForm(request.POST)
        print(("form = ", form))
        if form.is_valid():
            print(("form.cleaned_data = ", form.cleaned_data))
            form = form.save(commit=False)  # Presave the form
            form.farm_profile = farmprofile[0]  # Add the farmprofile ForeignKey
            previous_watering_time = WateringTime.objects.filter(farm_profile=farmprofile[0]).filter(flock=form.flock.id).last()
            print("previous_watering_time = ", previous_watering_time)
            if previous_watering_time:
                print("previous_watering_time.water_total = ", previous_watering_time.water_total)
                form.amount_water_consumed = previous_watering_time.water_total - form.amount_water_rem
                form.water_total = previous_watering_time.water_total - form.amount_water_consumed + form.amount_water_added
            form.save()
            return HttpResponseRedirect('/profile')
    else:
        form = WateringTimeForm
        flock = farmprofile[0].flocks.all()
        template = 'regular_tasks/watering_time.html'
        context = {'form': form,
                   'flocks': flock}
        return render(request, template, context)


def get_flock_waterings(request, flock_id):
    userprofile = UserProfile.objects.get(user=request.user)
    farmprofile = userprofile.farmprofiles.all()
    previous_flock_watering = list(WateringTime.objects.filter(flock=flock_id).values())
    print("previous_flock_watering = ", previous_flock_watering)
    # print("previous_flock_watering list = ", list(previous_flock_watering))
    return JsonResponse(list(previous_flock_watering), safe=False)
